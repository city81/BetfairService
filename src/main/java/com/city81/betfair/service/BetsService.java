...
public class BetsService {
	 ...
	 *            type of persistence eg IP - In Play
	 */
	public void placeBet(BetTypeEnum betType, int marketId, int selectionId,
		...
		if (stake < MINIMUM_STAKE) {
			if (betType.equals(BetTypeEnum.B)) {
				placeSmallStakeBackBet(marketId, selectionId, price, stake,
						betPersistenceType);
			} else {
				placeSmallStakeLayBet(marketId, selectionId, price, stake,
						betPersistenceType);
			}
		} else {
			place(betType, marketId, selectionId, price, stake,
					betPersistenceType);
		}
	 ...
	 *            old bet odds
	 * @param newPrice
	 *            new bet odds. Can be same as old bet odds.
	 * @param oldStake
	 *            old bet stake
	 * @param newStake
	 *            new bet stake. Can be same as old bet stake.
	 * @param oldBetPersistenceType
	 *            old bet persistence type
	 * @param newBetPersistenceType
	 *            new bet persistence type. Can be same as old bet stake.
	 */
	 ...
	 *            market identifier
	 * @param selectionId
	 *            selection identifier. Can be null.
	 */
	public void updateUnmatchedBets(Integer marketId, Integer selectionId) {

		ArrayOfBet betArray = null;
		List<Bet> bets = null;

		if ((betArray = getCurrentBets(BetStatusEnum.U, marketId, selectionId)) != null) {
			...
			for (Bet bet : bets) {
				...
				updateBet(bet.getBetId(), bet.getPrice(), newPrice,
						bet.getRemainingSize(), bet.getRemainingSize(),
	 ...
	 * 
	 * @param betStatus
	 *            status of bet eg U Unmatched
	 * @param marketId
	 *            market identifier. Can be null.
	 * @param selectionId
	 *            selection identifier. Can be null.
	 * @return ArrayOfBet an array of current bets. Can be null if no bets
	 *         found.
	 */
	public ArrayOfBet getCurrentBets(BetStatusEnum betStatus, Integer marketId,
		...
		GetCurrentBetsResp getMatchedBetsResp = null;
		
		boolean throttledExceeded = true;
		
		while (throttledExceeded) {
			
			getMatchedBetsResp = bfExchangeService.getCurrentBets(getMatchedBetsReq);
			...
			if ((getMatchedBetsResp.getHeader().getErrorCode() != null) &&
				...
				//System.out.println(getMatchedBetsResp.getErrorCode());
				//System.out.println(getMatchedBetsResp.getHeader().getErrorCode());
			} else {
	 ...
	 * @return double profit (or loss) for a particular selection
	 */
	public double getSelectionProfitLoss(Integer marketId, Integer selectionId) {

		double profitLoss = 0.0;
		double betProfitLoss = 0.0;
		
		ArrayOfBet betArray = null;
		List<Bet> bets = null;

		if ((betArray = getCurrentBets(BetStatusEnum.M, marketId, selectionId)) != null) {
				...
				if (selectionId != null) {
					if (bet.getSelectionId() == selectionId) {
						if (bet.getBetType() == BetTypeEnum.B) {
							betProfitLoss = (bet.getPrice() - 1.0) * bet.getMatchedSize();
						} else {
							betProfitLoss = -(bet.getPrice() - 1.0) * bet.getMatchedSize();
						}
						profitLoss = profitLoss + betProfitLoss;
						//System.out.println(bet.getBetId() + " " + betProfitLoss);
					}
				}
			}
		}
	...
	private void placeSmallStakeBackBet(int marketId, int selectionId,
		...
		// stake. This will result in two bets on the market ie the original
		// minimum
	...
	private long place(BetTypeEnum betType, int marketId, int selectionId,
		...
		return placeBetsResp.getBetResults().getPlaceBetsResult().get(0)
				.getBetId();
	...
	private long update(long betId, double oldPrice, double newPrice,
		...
		return updateBetsResp.getBetResults().getUpdateBetsResult().get(0)
				.getNewBetId();
	}
